...
namespace WebservicesSage.Cotnroller
{
    public static class ControllerCommande
    {
        ...
        {
            CheckForNewOrderMagentoFlag1();
            CheckForNewOrderMagentoFlag3();
        }

        public static void CheckForNewOrderMagentoFlag3()
        {

            try
            {
                File.AppendAllText("Log\\orderFlag.txt", Environment.NewLine + "Upload order with flag 3" + Environment.NewLine);
                string response = UtilsWebservices.SearchOrder(UtilsConfig.BaseUrl + "/rest/V1/orders", UtilsWebservices.SearchOrderCriteria("order_flag", "3", "eq"));
                ...
                if (orderSearch.TotalCount > 0)
                {
                    ...
                    for (int i = 0; i < orderSearch.TotalCount; i++)
                    {
                        ...
                        try
                        {
                            ...
                            if (
                                (orderSearch.Items[i].Status.Equals("canceled") || orderSearch.Items[i].State.Equals("canceled"))
                            ...
                            if (
                                orderSearch.Items[i].Status.Equals("pending_payment")
                                || orderSearch.Items[i].State.Equals("pending_payment")
                            ...
                            if (!String.IsNullOrEmpty(clientCtNum.ToUpper()))
                            {
        ...
        /// <param name="e"></param>
        public static void CheckForNewOrderMagentoFlag1()
        {
        ...
        public static void AddNewOrderForCustomer(OrderSearchItem orderMagento, string CT_Num,CustomerSearch customerMagento)
        {
            ...
            try
            {
                ...
                if (orderMagento.ExtensionAttributes.ShippingAssignments[0].Shipping.Method.Equals("dpd_predict"))
                {
                    carrier_id = "1";
                    
                }
                if (orderMagento.ExtensionAttributes.ShippingAssignments[0].Shipping.Method.Equals("colissimo_homecl")|| orderMagento.ExtensionAttributes.ShippingAssignments[0].Shipping.Method.Equals("colissimo_homesi"))
                {
                    carrier_id = "2";
                }
                if (orderMagento.ExtensionAttributes.ShippingAssignments[0].Shipping.Method.Equals("flatrate_flatrate"))
                {
                    carrier_id = "3";
                }
                if (orderMagento.ExtensionAttributes.ShippingAssignments[0].Shipping.Method.Equals("dpd_pickup"))
                {
                    carrier_id = "4";
                }
            ...
            if (orderMagento.Payment.Method.Equals("checkmo") || orderMagento.Payment.Method.Equals("banktransfer") )
            {
            ...
            try
            {
                if (orderMagento.Payment.Method.Equals("sogecommerce_standard"))
                {
                    IBODocumentAcompte3 acompte = (IBODocumentAcompte3)order.FactoryDocumentAcompte.Create();
                    acompte.DR_Montant = orderMagento.BaseTotalDue;
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append(DateTime.Now + "probleme Acompte : "+orderMagento.IncrementId.ToString() + Environment.NewLine);
            ...
            try
            {
                ...
                string frais = orderMagento.ExtensionAttributes.ShippingAssignments[0].Shipping.Total.ShippingAmount;
                // reffrais = "PORTEMB1";
            ...
            {
                if (e.Message.ToString().Equals("Cet élément est en cours d'utilisation !"))
                {
                    StringBuilder erreur = new StringBuilder();
                    erreur.Append(DateTime.Now + "problem Document line" + Environment.NewLine);
                    erreur.Append(DateTime.Now + e.Message + Environment.NewLine);
                    erreur.Append(DateTime.Now + e.StackTrace + Environment.NewLine);
