    ...
    public partial class MainUI : Form
    {
        ...
        private void SynchCategories_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                IBOClient3 customer = gescom.CptaApplication.FactoryClient.ReadNumero("CARAT");
                customer.CT_Intitule = customer.CT_Intitule + "test";
                customer.Write();
                ...
                order.DO_Ref = "test nomenclature";
                order.SetDefaultDO_Piece();
                order.Write();
                IBODocumentVenteLigne3 docLigne = (IBODocumentVenteLigne3)order.FactoryDocumentLigne.Create();
                var ArticleExist = gescom.FactoryArticle.ExistReference("CFI-1");
                if (ArticleExist)
                {
                    IBOArticle3 article1 = gescom.FactoryArticle.ReadReference("CFI-1");
                    Object.Article CondArticle = new Object.Article(article1);
                    // insertion des article à nomenclature
                    if (CondArticle.HaveNomenclature)
                    {
                        docLigne.SetDefaultArticle(article1, Int32.Parse("3"));
                        ...
                        foreach (IBOArticleNomenclature3 item in article1.FactoryArticleNomenclature.List)
                        {
                            ...
                            docligneComposant.SetDefaultArticle(item.ArticleComposant, item.NO_Qte *3);
                ...
                IBODocumentVenteLigne3 docLignePort = (IBODocumentVenteLigne3)order.FactoryDocumentLigne.Create();
                string frais = "10";
                // reffrais = "PORTEMB1";
